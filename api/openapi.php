    ...
    'schemas' => [
        ...
        'Institucion' => [
            ...
            'properties' => [
                ...
                'nombre'          => ['type' => 'string', 'maxLength' => 100, 'example' => 'Unidad Educativa Particular Nuestra Señora del Rosario'],
                'direccion'       => ['type' => 'string', 'maxLength' => 100],
                'telefono'        => ['type' => 'string', 'maxLength' => 50, 'example' => '(04) 2345-678 ext. 102'],
                'estado'          => ['type' => 'string', 'enum' => ['ACTIVO', 'INACTIVO']],
            ],
        ],
        ...
        'InstitucionEntrada' => [
            ...
            'properties' => [
                'id'              => ['type' => 'integer', 'readOnly' => true, 'description' => 'No se recibe: lo asigna la base con AUTO_INCREMENT.'],
                'nombre'          => ['type' => 'string', 'maxLength' => 100],
                'direccion'       => ['type' => 'string', 'maxLength' => 100],
                'telefono'        => ['type' => 'string', 'maxLength' => 50, 'example' => '(04) 2345-678 ext. 102', 'description' => 'Contacto telefónico de la INSTITUCIÓN, no de una persona: admite letras, extensiones o más de un número, y solo se le exige llevar algún dígito. El teléfono de una persona sigue otra regla —solo dígitos, 16 caracteres—; ver api/core/Telefono.php.'],
        ...
        'Usuario' => [
            ...
            'properties' => [
                ...
                'InstitucionNombre'      => ['type' => 'string', 'description' => 'Institución a la que PERTENECE la cuenta. Se devuelve al SuperAdmin, que ve toda la red.'],
                'EsPropia'               => ['type' => 'boolean', 'description' => 'La cuenta pertenece a la institución activa. Si es falso, viene de otra y solo se le pueden mover los roles de aquí.'],
                'es_propia'              => ['type' => 'boolean', 'description' => 'Igual que EsPropia, en la consulta individual.'],
                'puede_editar_identidad' => ['type' => 'boolean', 'description' => 'Cierto si quien consulta puede cambiar el nombre de usuario, el estado y la contraseña; falso cuando solo alcanza a los roles de la institución activa.'],
...
$spec['paths']['/documento/reglas'] = [
    'get' => [
        ...
        'description' => "Qué admite cada tipo de documento y hasta dónde llega la columna de la base de datos.\n\n`CEDULA` y `RUC` solo admiten dígitos, con un largo exacto de 10 y 13. Se comprueba la forma, no la validez del número: el dígito verificador no se revisa. `PASAPORTE` admite además letras y no tiene un formato internacional único, así que lo que se le exige es un rango: entre 6 y 12 caracteres. El largo de la columna actúa como último techo y se lee de `persona`.`Identificacion`, o de la más corta entre esa y `proveedor`.`Ruc` cuando el contexto es `proveedor`.\n\nCada tipo viaja con `exacto`, `minimo` y `maximo`: `exacto` en cero significa que no hay una medida única y que manda el rango.\n\nEn la metadata viajan también las reglas del **teléfono** y las del **correo**, que son los otros dos campos que captura el mismo formulario.\n\nLo consumen los formularios para adaptar el campo y avisar mientras se escribe. **Quien decide sigue siendo el servidor** al guardar: esto es una comodidad para quien captura, no un control.\n\n**Acceso:** autenticado.",
